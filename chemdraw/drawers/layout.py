...
class ConfigLayout:
    ...
    def get_scaling(self, molecule: Molecule, label: str):
        if self.range_x is None or self.range_y is None:
            ...
            if self.parent.label.show and label is not None:
                title_height = self.parent.label.text_box_size(label)
                if self.parent.label.location == "top":
                    max_ += title_height
                else:
                    min_ -= title_height
